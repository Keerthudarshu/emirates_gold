...
$type = isset($_GET['type']) ? $_GET['type'] : 'gold_chain';
$row = null;
$source = '';

if ($type === 'gold_chain') {
    if ($code) {
        $sql = "SELECT * FROM gold_chain WHERE code = '" . $conn->real_escape_string($code) . "'";
        ...
        $sql = "SELECT * FROM gold_chain WHERE id = $id";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'gold_chain';
        ...
        <div class="enquiry-popup">
            ...
            <div class="popup-header">
                ...
                <div class="product-info">
                    <small>Enquiry for: <?php echo htmlspecialchars($row['name'] ?? 'Gold Chain'); ?> - <?php echo htmlspecialchars($row['code'] ?? ''); ?></small>
                </div>
            </div>
