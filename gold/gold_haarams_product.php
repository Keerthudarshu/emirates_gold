...
$type = isset($_GET['type']) ? $_GET['type'] : 'gold_haarams';
$row = null;
$source = '';

if ($type === 'gold_haarams') {
    if ($code) {
        $sql = "SELECT * FROM gold_haarams WHERE code = '" . $conn->real_escape_string($code) . "'";
        ...
        $sql = "SELECT * FROM gold_haarams WHERE id = $id";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'gold_haarams';
        ...
        <div class="enquiry-popup">
            ...
            <div class="popup-header">
                ...
                <div class="product-info">
                    <small>Enquiry for: <?php echo htmlspecialchars($row['name'] ?? 'Gold Haarams'); ?> - <?php echo htmlspecialchars($row['code'] ?? ''); ?></small>
                </div>
            </div>
