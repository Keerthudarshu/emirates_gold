...
$type = isset($_GET['type']) ? $_GET['type'] : 'diamond_bangles';
$row = null;
$source = '';

if ($type === 'diamond_bangles') {
    if ($code) {
        $sql = "SELECT * FROM diamond_bangles WHERE code = '" . $conn->real_escape_string($code) . "'";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'diamond_bangles';
        }
    } elseif ($id) {
        $sql = "SELECT * FROM diamond_bangles WHERE id = $id";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'diamond_bangles';
...
<body data-rsssl=1 class="home wp-singular page-template-default page page-id-2374 wp-embed-responsive wp-theme-hello-elementor wp-child-theme-muliya-jewels theme-hello-elementor woocommerce-no-js hello-elementor-default elementor-default elementor-kit-7 elementor-page elementor-page-2374">
    ...
    <div class="product-view-container">
        ...
        <div class="product-details">
            ...
            $backLinks = [
                'products' => 'index.php',
                'diamond_bangles' => 'diamond_bangles.php'
            ];
        ...
        <div class="enquiry-popup">
            ...
            <div class="popup-header">
                ...
                <div class="product-info">
                    <small>Enquiry for: <?php echo htmlspecialchars($row['name'] ?? 'Diamond Bangles'); ?> - <?php echo htmlspecialchars($row['code'] ?? ''); ?></small>
                </div>
            </div>
