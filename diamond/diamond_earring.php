  ...
  <?php include_once '../header.php'; ?>
    ...
    <div class="product-list">
       <?php
        ...
        $count_sql = "SELECT COUNT(*) as total FROM diamond_earring";
        ...
        $sql = "SELECT * FROM diamond_earring LIMIT $items_per_page OFFSET $offset";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $productUrl = 'diamond_earring_product.php?code=' . urlencode($row['code']) . '&type=diamond_earring';
                echo '<div class="product-card">';
                echo '<a href="' . $productUrl . '">';
                $imgPath = !empty($row['image']) ? $row['image'] : 'img/diamond_earring/default.jpg';
            ...
            echo "<p>No gold earring products found.</p>";
        }
        ?>
    </div>
