  ...
  <?php include_once '../header.php'; ?>
    ...
    <div class="product-list">
       <?php
        ...
        $count_sql = "SELECT COUNT(*) as total FROM diamond_necklaces";
        ...
        $sql = "SELECT * FROM diamond_necklaces LIMIT $items_per_page OFFSET $offset";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $productUrl = 'diamond_necklaces_product.php?code=' . urlencode($row['code']) . '&type=diamond_necklaces';
                echo '<div class="product-card">';
                echo '<a href="' . $productUrl . '">';
                $imgPath = !empty($row['image']) ? $row['image'] : 'img/diamond_necklaces/default.jpg';
                echo '<img src="' . htmlspecialchars($imgPath) . '" alt="' . htmlspecialchars($row['name']) . '" style="max-width:100%;height:auto;">';
                echo '</a>';
            ...
            echo "<p>No diamond necklaces products found.</p>";
        }
        ?>
    </div>
