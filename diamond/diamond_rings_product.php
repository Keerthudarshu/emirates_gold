...
$type = isset($_GET['type']) ? $_GET['type'] : 'diamond_rings';
$row = null;
$source = '';

if ($type === 'diamond_rings') {
    if ($code) {
        $sql = "SELECT * FROM diamond_rings WHERE code = '" . $conn->real_escape_string($code) . "'";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'diamond_rings';
        }
    } elseif ($id) {
        $sql = "SELECT * FROM diamond_rings WHERE id = $id";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $source = 'diamond_rings';
  ...
  <?php include_once '../header.php'; ?>
    ...
    <div class="product-view-container">
        ...
        <div class="product-details">
            ...
            $backLinks = [
                'products' => 'index.php',
                'diamond_rings' => 'diamond_rings.php',
            ];
            $backHref = isset($backLinks[$source]) ? $backLinks[$source] : 'diamond_rings.php';
            ?>
            <a class="back-link box-btn" href="<?php echo $backHref; ?>">&larr; Back to Products</a>
        </div>
    </div>
        ...
        <div class="enquiry-popup">
            ...
            <div class="popup-header">
                ...
                <div class="product-info">
                    <small>Enquiry for: <?php echo htmlspecialchars($row['name'] ?? 'Diamond Ring'); ?> - <?php echo htmlspecialchars($row['code'] ?? ''); ?></small>
                </div>
            </div>
