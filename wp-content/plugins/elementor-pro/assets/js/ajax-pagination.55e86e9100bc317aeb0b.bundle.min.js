        ...
        4734: (e, t, n) => {
            ...
            class AjaxPagination extends elementorModules.frontend.handlers.Base {
                ...
                handleSuccessFetch(e) {
                    ...
                    const t = e.querySelector(`[data-id="${this.elementId}"]`),
                        n = document.querySelector(`[data-id="${this.elementId}"]`);
                    if (!t || !n) return;
                    const s = n.parentNode;
                    s && s.replaceChild(t, n), this.afterInsertPosts()
                }
                afterInsertPosts() {
                    const e = this.getSettings("selectors"),
                        t = document.querySelectorAll(`[data-id="${this.elementId}"] ${e.postWrapperTag}`);
                    elementorFrontend.elementsHandler.runReadyTrigger(document.querySelector(`.elementor-element-${this.elementId}`)), (0, r.default)(t), ElementorProFrontendConfig.settings.lazy_load_background_images && document.dispatchEvent(new Event("elementor/lazyload/observe"))
                }
                onInit() {
